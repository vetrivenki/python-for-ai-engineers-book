...
Run:  python claude_cli.py [--stream] [--system "..."] [--max-tokens 400]
      python claude_cli.py --fake            (scripted offline transcript)
...
from chat_loop import ChatSession
from config import API_KEY_VAR, DEFAULT_MODEL, api_key_present
from measured_client import Record, emit, measure
from resilient_client import PermanentError, TransientError, call_with_retry
...
def main(argv: list[str]) -> int:
    ...
    if not args.fake and not api_key_present():
        print(f"[fatal] {API_KEY_VAR} is not set. Run setup_check.py first. No request sent.")
        return 1
