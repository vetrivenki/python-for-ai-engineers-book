...
Run:  python test_client.py            (no dependencies)
      python -m pytest test_client.py  (if pytest is installed)
"""
...
import claude_cli
from chat_loop import ChatSession, fake_sender
from config import API_KEY_VAR
from measured_client import fake_failure, fake_success, measure
from resilient_client import (MAX_ATTEMPTS, PermanentError, TransientError,
                              call_with_retry, make_fake)
from stream_response import StreamInterrupted, consume, fake_stream
...
def test_first_message_parses_non_text_block() -> None:
    from first_message import FakeResponse, describe, extract_text
